...
- `mc_bowser-xxx-offline.jar`: 附带了JCEF插件的版本，你仍然需要下载前置mod Rinku，但是Rinku无需联网下载JCEF，适用于国内及网络不好的玩家。嵌入式压缩包在使用前会通过 SHA-256 校验和 Rinku 的完整清单进行验证。当你首次安装并成功启动游戏后，离线版会将JCEF插件缓存释放到你的游戏根目录中，后续更新即可删除离线版mod，直接将新版本普通包放入mod文件夹即可，不会影响已生成的插件缓存调用机制，以此来降低你的存储占用。

- 如果你需要在服务器安装，服务器只需安装普通版本jar且不需要安装前置Rinku，客户端仍需按上述方式安装mod。
